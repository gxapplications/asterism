...
export default class CommandManager {
  constructor (annyang, continuousMode, mainState, speak, setLanguage, continueDialog, stopDialog, logger, commandGenerators) {
  ...
  get rootCommands () {
    ...
    this.rootCommandsCache = {
      ...this.commandGenerators.reduce((acc, cg) => {
        acc = {
          ...cg({
            language: this.language,
            logger: this.logger,
            buildNext: this.buildNextCommand.bind(this),
            buildYesNo: this.buildYesNo.bind(this),
            speak: this.speak,
            thesaurus: this.getThesaurus.bind(this),
            reduceCommands: this.reduceCommands.bind(this)
          }),
          ...acc
        }
        return acc
