...
class ItemManager {
  ...
  getAllItems () {
    ...
    return this.orderHandler.getLocalOrder().map(({ id, w, h }) => {
      const item = getFromId(id, factories, false, this.settingPanelClosed)
      if (item instanceof Promise) {
  ...
  addNewItem (id, item, h, w, settingPanel) {
  ...
  settingPanelClosed (data) {
    const { instanceId, item, preferredHeight, preferredWidth, settingPanel } = data

    $('#item-setting-modal').modal('close')
    // TODO !4: animation from modal to [new] item in the grid ?
    sleep(400)
    .then(() => {
      if (data) {
        // new item to add
        this.addNewItem(instanceId, item, preferredHeight, preferredWidth, settingPanel)
      }
