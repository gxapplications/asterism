...
class DefaultServerStorage {
  ...
  getLastItemByT (quantity) {
    return this.fetchJson(`/data-last-by-t/${this.prefix}/${quantity}`, { method: 'GET' })
  }

  getItemsAfterDate (timestamp, fields = null, limit = 100) {
    const query = `?l=${limit || 100}` + (fields && fields.length) ? `&f=${fields.join('&f=')}` : ''
    return this.fetchJson(`/data-after-date/${this.prefix}/${timestamp}${query}`, { method: 'GET' })
  }
