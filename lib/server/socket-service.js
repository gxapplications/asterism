...
let _heartbeat = null
const _manageAsterismSocket = (socketIo, server, notificationService) => (socket) => {
  ...
  socket.on('halt-system', (ack) => {
    ack() // ack first, because next step will stop server and exit process
    server.stop(() => {
      process.exit(0)
    }, 'Halt system asked from browser.')
  })
...
export default class SocketService {
  ...
  connect (server) {
    ...
    asterismIo.on('connect', _manageAsterismSocket(asterismIo, server, this.notificationService))
