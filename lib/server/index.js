...
class Server extends EventEmitter {
  constructor () {
    ...
    this.dataHandler = new DataHandler('asterism')
  ...
  use (pluginManifest) {
    ...
    try {
      ...
      if (pluginManifest.developmentLibName) {
        this.browserWebpackIncludeMask.push(pluginManifest.developmentLibName)
      }
    } catch (error) {
  ...
  start (port, ipWhiteList, callback) {
    this.express.use(IpFilter(ipWhiteList, {mode: 'allow'}))

    const server = this.express.listen(port, () => {
      this.emit('start', callback)
      console.log(`Express server ready on port ${port}. Waiting for webpack to finish...`.grey)
    })
  ...
  _serverPlug (pluginManifest) {
    ...
    const context = {
      logger: this.logger.createSubLogger(pluginManifest.name),
  ...
  stop (callback = () => {}, msg = null) {
    // TODO !5: send kill signal to data/socket services

    console.log('Disconnecting middlewares...'.cyan)
    this.middlewares.forEach((middleware) => {
      if (middleware.disconnect) {
