...
class Server extends EventEmitter {
  constructor (webPushParams = null) {
    super()
    console.log(`Asterism booting on server '${(webPushParams && webPushParams.webPushServerUrl) || 'localhost'}'... (${version})`.cyan)
    this.webPushServerUrl = webPushParams && webPushParams.webPushServerUrl
  ...
  start (portOffset, ipWhiteList, callback, greenlock = null) {
    ...
    if (greenlock) {
      ...
      mainUrl = this.webPushServerUrl || mainUrl
...
Server.getWebPushParameters = () => {
  ...
  if (publicVapidKey && privateVapidKey && email && serverUrl) {
    ...
    return { serverWebPush: { email, publicVapidKey, privateVapidKey, serverUrl }, browserWebPush: { publicVapidKey } }
  }

  try {
    const { email, publicVapidKey, privateVapidKey } = require('../../var/webPush.json')
    return { serverWebPush: { email, publicVapidKey, privateVapidKey, serverUrl: null }, browserWebPush: { publicVapidKey } }
  } catch (error) {
