...
export default class NotificationService {
  ...
  router () {
    ...
    router.post('/web-push-subscribe', (req, res) => {
      ...
      if (this.webPushSubscriptions.find(s => s.endpoint === subscription.endpoint)) {
        this.webPushSubscriptions = this.webPushSubscriptions.map(s => (s.endpoint === subscription.endpoint) ? subscription : s)
        this.dataHandler.setItem('web-push-subscriptions', this.webPushSubscriptions)
        .then(() => {
