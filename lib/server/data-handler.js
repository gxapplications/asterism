...
export default class DataHandler {
      ...
      db.find().make((builder) => {
        builder
          ...
          .callback((err, result) => {
            ...
            // TODO !2: deactivated because seems not used. To removed!
  ...
  getItemsByParentId (parentId, forcePrefix, scanlimit = 1000) {
    const db = forcePrefix ? _dbs[forcePrefix] : this.db
    return new Promise((resolve, reject) => {
      db.find().make((builder) => {
        builder
          //.query(`doc.item.parent===${parentId}`)
          .take(+scanlimit || 1000)
          .callback((err, result) => {
            if (err) {
              this.logger.error('DATA GET ERROR', err)
            ...
            const filtered = result?.filter((e) => e.item.parent === parentId)
