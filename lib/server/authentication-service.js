...
const tokenTTL = 12 * 3_600_000 // 12 hours
const longTokenTTL = tokenTTL * 24 * 2 // 24 days
    ...
    const redirectIfNotAuth = () => {
      ...
      router.post('/login', (req, res, next) => {
        ...
        return this.testPassword(encodedPassword)
          .then((scope) => {
            ...
            res.json({ url: decodeURIComponent(req.query.then || '/') })
          })
      })
      ...
      router.all('/login', (req, res) => {
        return this.isAuthenticationEnabled()
          .then((enabled) => {
            ...
            res.clearCookie('readOnly-access-token')
            res.clearCookie('password-encoder')
            res.cookie('password-encoder', this.passwordEncoder, { maxAge: 180 * 60_000, signed: false })
      ...
      router.all('*', (req, res, next) => {
        if (req.url.startsWith('/favicon.ico') ||
          ...
          req.url.startsWith('/asterism-scenarii/trigger/')) {
          return next()
        }
