...
const schema = Joi.object().keys({
  ...
  // TODO !4: should we abort executing warning actions when deactivating a raised alarm?
  alarmingActions: Joi.array().min(0).max(16).required().items(elementId).default([]),
  // TODO !4: should we abort executing alarming actions when deactivating a raised alarm?
  deactivationActions: Joi.array().min(0).max(16).required().items(elementId).default([])

  // TODO !4: more attributes?
})
