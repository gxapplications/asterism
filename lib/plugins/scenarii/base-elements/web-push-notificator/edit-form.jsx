  ...
  render () {
    ...
    return (
      <Row className='section card form webPushNotificatorPanel'>
        ...
        <TextInput
          placeholder='From Asterism' s={12} m={8} label='Notification title' defaultValue={title} data-length={64}
          onChange={(e) => { instance.data.title = e.currentTarget.value.substring(0, 64); this.props.highlightCloseButton() }}
        />
        <div className='col s12'>&nbsp;</div>

        <TextInput
          placeholder='Hello world!' s={12} label='Notification content' defaultValue={body}
          onChange={(e) => { instance.data.body = e.currentTarget.value; this.props.highlightCloseButton() }}
        />
        <div className='col s12'>&nbsp;</div>

        <TextInput
          placeholder='(advanced use)' s={12} label='Unicity identifier' defaultValue={unicity} data-length={32}
