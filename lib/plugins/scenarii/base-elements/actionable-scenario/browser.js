...
class BrowserActionableScenario extends BrowserScenario {
  ...
  presave (services) {
    ...
    return Promise.all([
      ...
      this.executionTrigger ? Promise.resolve(this.executionTrigger) : scenariiService.getTriggerInstance(this.data.executionTrigger, true),
      ...
      .then(([trigger, condition, action]) => {
        ...
        if (this.executionTrigger && trigger.parent && trigger.parent.length > 0) {
