'use strict'
/* eslint-disable no-labels, no-case-declarations */
...
const _getWaitDuration = (data, now = moment()) => {
  ...
  l1: switch (data.dayMode) {
    ...
    case 'dayAndMonth':
      ...
      for (const [day, month] of dates.slice(1)) {
        if (m.isBefore(immediately)) {
          m.month(month)
          m.date(day)
          nextDay.month(month)
  ...
  l2: switch (data.timeMode) {
    case 'hourMinute':
      ...
      console.error('Time based trigger: _getWaitDuration, failed to schedule in hourMinute case. Now:', now.format(), ' m:', m.format(), ' immediately:', immediately.format())
    ...
    case 'eachQuarter':
      ...
      console.error('Time based trigger: _getWaitDuration, failed to schedule in eachQuarter case. Now:', now.format(), ' m:', m.format(), ' immediately:', immediately.format())
      return null // cannot schedule... but cannot occurs I think.
    ...
    case 'eachHalf':
      ...
      console.error('Time based trigger: _getWaitDuration, failed to schedule in eachHalf case. Now:', now.format(), ' m:', m.format(), ' immediately:', immediately.format())
      return null // cannot schedule... but cannot occurs I think.
    ...
    case 'eachHour':
      ...
      console.error('Time based trigger: _getWaitDuration, failed to schedule in eachHour case. Now:', now.format(), ' m:', m.format(), ' immediately:', immediately.format())
      return null // cannot schedule... but cannot occurs I think.
  }
...
export default class ServerTimeBasedTrigger extends ServerTrigger {
  ...
  scheduleEvents () {
    try {
      ...
      if (duration) {
        this.schedule = duration.timer({ start: true }, () => {
          this.listeners.forEach((listener) => listener())
          setTimeout(this.reschedule.bind(this), 2000)
        })
        return Promise.resolve(true)
      }
