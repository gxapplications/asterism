...
const _getElementInstance = (dataHandler, elementType, typeRepository, instanceId) => {
  ...
  return dataHandler.getItem(instanceId)
    .then((instance) => {
      ...
      const { typeId, type, data, parent, group } = instance
      if (type !== elementType) {
      ...
      return dataSchema.validateAsync(data)
        .catch((error) => {
...
const _createElementInstance = (logger, elementType, typeRepository, typeId, defaultData, defaultId) => {
  ...
  return dataSchema.validateAsync(defaultData)
    .catch((error) => {
      throw new _ValidationError(error.message)
    })
    .then((fixedData) => {
...
const _setElementInstance = (logger, dataHandler, elementType, typeRepository, element, parent = undefined, group = undefined) => {
  ...
  return type[1].dataSchema.validateAsync(element.data)
    .catch((error) => {
      throw new _ValidationError(error.message)
    })
    .then((fixedData) => {
...
export default class ScenariiService {
  constructor ({ getServices, logger, dataHandler, notificationHandler, privateSocket, publicSockets }) {
    ...
    baseElements({ logger, scenariiService: this, dataHandler, notificationHandler }).forEach(({ id, serverClass, dataSchema }) => {
    ...
    this.getTriggerInstances().then((instances) => {
      return Promise.all(instances.map(({ typeId, instanceId }) => {
        return _getElementInstance(this.dataHandler, 'trigger', this.triggers, instanceId)
          .then((instance) => {
            this.logger.error(`Mounted trigger ${instance.data.name} of type ${typeId} into memory.`) // TODO !0: turn back log into info()
  ...
  getActionInstances (parent = undefined) {
    return this.dataHandler.listItems((i) => i.item.type === 'action' && i.item.parent == parent) // eslint-disable-line eqeqeq
      .then((results) => results.map((i) => ({ typeId: i.item.typeId, instanceId: i.keyName, data: i.item.data, parent: i.item.parent, group: i.item.group })))
  }
  ...
  setActionData (actionData, parent, group) {
    return _getElementInstance(this.dataHandler, 'action', this.actions, actionData.instanceId)
      ...
      .then((action) => {
        return _setElementInstance(this.logger, this.dataHandler, 'action', this.actions, action, parent, group)
      })
  }

  setActionInstance (action) {
    return _setElementInstance(this.logger, this.dataHandler, 'action', this.actions, action, action.parent, action.group)
  }
  ...
  actionExecutionState (executionId) {
    return Promise.resolve(!!this.actionExecutions[executionId])
  }
  ...
  setConditionData (conditionData, parent = undefined, group = undefined) {
    return _getElementInstance(this.dataHandler, 'condition', this.conditions, conditionData.instanceId)
      ...
      .then((condition) => _setElementInstance(this.logger, this.dataHandler, 'condition', this.conditions, condition, parent, group))
  }

  setConditionInstance (condition) {
    return _setElementInstance(this.logger, this.dataHandler, 'condition', this.conditions, condition, condition.parent, condition.group)
  }
  ...
  getTriggerInstances (parent = undefined) {
    // parent === undefined => get all
    // parent === null => get global instances only
    return this.dataHandler.listItems((i) => i.item.type === 'trigger' && ((parent === undefined) || (i.item.parent === parent)))
      .then((results) => results.map((i) => ({ typeId: i.item.typeId, instanceId: i.keyName, data: i.item.data, parent: i.item.parent, group: i.item.group })))
  }
  ...
  getTriggerInstance (instanceId, asObject = false) {
    ...
    return Promise.resolve(
      asObject
      ? instance
      : { typeId: instance.typeId, instanceId: instance.instanceId, data: instance.data, group: instance.group, parent: instance.parent })
  }
  ...
  setTriggerData (triggerData, parent = undefined, group = undefined) {
    return _getElementInstance(this.dataHandler, 'trigger', this.triggers, triggerData.instanceId)
      ...
      .then((trigger) => {
        ...
        instanceToUpdate.parent = parent
        instanceToUpdate.group = group
        return _setElementInstance(this.logger, this.dataHandler, 'trigger', this.triggers, trigger, parent, group)
          .then((succeed) => {
            if (succeed) {
  ...
  setTriggerInstance (trigger) {
    return _setElementInstance(this.logger, this.dataHandler, 'trigger', this.triggers, trigger, trigger.parent, trigger.group)
      .then((succeed) => {
        if (succeed) {
  ...
  getScenarioInstances () {
    return this.dataHandler.listItems((i) => i.item.type === 'scenario')
      .then((results) => results.map((i) => ({ typeId: i.item.typeId, instanceId: i.keyName, data: i.item.data, group: i.item.group })))
  }
  ...
  getScenarioInstance (instanceId) {
    ...
    return Promise.resolve({ typeId: instance.typeId, instanceId: instance.instanceId, data: instance.data, group: instance.group })
  }
  ...
  createScenarioInstance (typeId) {
    return _createElementInstance(this.logger, 'scenario', this.scenarii, typeId)
      .then((instance) => {
        ...
        return { typeId: instance.typeId, instanceId: instance.instanceId, data: instance.data, group: instance.group }
      })
  }

  setScenarioData (scenarioData, group = undefined) {
    return _getElementInstance(this.dataHandler, 'scenario', this.scenarii, scenarioData.instanceId)
      ...
      .then((scenario) => {
        ...
        return { typeId: instanceToUpdate.typeId, instanceId: instanceToUpdate.instanceId, data: instanceToUpdate.data, group: instanceToUpdate.group }
      // no need to set it in DB here : the triggered listener will make it! (else can be added twice...)
      // return _setElementInstance(this.logger, this.dataHandler, 'scenario', this.scenarii, scenario, undefined, group)
      })
  }

  setScenarioInstance (scenario) {
    return _setElementInstance(this.logger, this.dataHandler, 'scenario', this.scenarii, scenario, undefined, scenario.group)
      .then((succeed) => {
        if (succeed) {
  ...
  deleteScenarioInstance (scenario) {
    ...
    delete instanceToDelete.listeners // avoid listeners to be called after deletion
    instanceToDelete.destroy && instanceToDelete.destroy()
    this.dataHandler.getItemsByParentId(scenario.instanceId).then((elementsToDelete) => {
      if (elementsToDelete) {
        elementsToDelete.forEach((e) => {
          if (e.item.type === 'trigger') {
            return this.deleteTriggerInstance({ instanceId: e.keyName })
          }
          if (e.item.type === 'state') {
            return this.deleteStateInstance({ instanceId: e.keyName })
          }
          if (e.item.type === 'scenario') {
            return this.deleteScenarioInstance({ instanceId: e.keyName })
          }
          _deleteElementInstance(this.logger, this.dataHandler, e.keyName)
        })
      }
    })
  ...
  setActivationScenarioInstance (scenarioData, activate, immediately = false) {
    this.logger.info(`Activating scenario ${scenarioData.name || scenarioData.instanceId} with value ${!!activate}, immediately ${!!immediately}.`)
    const instanceToUpdate = this.scenarioInstances.find((i) => i.instanceId === scenarioData.instanceId)
    return instanceToUpdate.requestSoftActivation(activate, immediately) // to trigger scenario listener broadcasting
      .then(() => instanceToUpdate)
      .catch((error) => {
        this.logger.error(error)
        return null
      })
  ...
  httpTrigger (req, res) {
    return Promise.allSettled(this.httpListeners.map((listener) => listener(req)))
      .then((results) => {
        results = results.filter((result) => result.value !== false) // remove triggers that does not match path
        const text = results.map((result) => result.value).join('\n') // get resulting texts (oks AND errors)
        const status = results.some((result) => result.status === 'rejected') ? 500 : 200
        res.status(status).send(text || '∅')
      })
  }

  addHttpListener (listener) {
    listener.httpListenerId = uuid.v4()
    this.httpListeners.push(listener)
    return listener.httpListenerId
  }

  removeHttpListener (listenerId) {
    this.httpListeners = this.httpListeners.filter((listener) => listener.httpListenerId !== listenerId)
  }
}
